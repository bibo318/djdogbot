...
class DejenTod:
    ...
    def http(self, url, headers, data=None):
        while True:
            try:
                ...
                self.log(f"{merah}lỗi kết nối/hết thời gian kết nối !")
                time.sleep(1)
                continue
    ...
    def countdown(self, t):
        for i in range(t, 0, -1):
            ...
            jam = str(jam).zfill(2)
            print(f"{putih}Chờ {jam}:{menit}:{detik} ", flush=True, end="\r")
            time.sleep(1)
    ...
    def is_expired(self, token):
        ...
        if now > jeload["exp"]:
            self.log(f"{merah}mã thông báo đã hết hạn !")
            return True
    ...
    def login(self, data):
        ...
        if res.status_code != 200:
            self.log(f"{merah}mã thông báo tìm nạp không thành công, kiểm tra http.log !")
            return None

        token = res.json()["data"]["accessToken"]
        self.log(f"{hijau}thành công nhận được token !")
        return token
    ...
    def account(self):
        ...
        while True:
            ...
            if res.status_code != 200:
                self.log(f"{merah}không lấy được thông tin tài khoản !")
                return False
            ...
            if adopted is False:
                ...
                if res.status_code != 200:
                    self.log(f"{merah}thông qua thất bại dog !")
                    return False

                self.log(f"{hijau}thông qua thành công {biru}dog")
                continue
        ...
        while True:
            ...
            if res.status_code != 200:
                self.log(f"{merah}thông tin thanh tìm nạp không thành công !")
                return
            ...
            self.log(f"{hijau}năng lượng sẵn có : {putih}{avail}")
            self.log(f"{hijau}tổng số vàng : {putih}{gold}")
            if zawarudo:
                return
            while True:
                ...
                if res.status_code != 200:
                    self.log(f"{merah}nhấp chuột gửi không thành công !")
                    continue
                ...
                avail -= click
                self.log(
                    f"{hijau}gửi thành công {putih}{click} {hijau}click{biru},{hijau}Năng lượng : {putih}{avail}"
                )
                if avail <= 0:
                    break
            ...
            if self.auto_upgrade:
                while True:
                    ...
                    if gold > price_levelup:
                        ...
                        retcode = res.json().get("returnCode")
                        if retcode != 200:
                            self.log(f"{merah}thăng cấp thất bại !")
                            break
                        self.log(f"{biru}thăng cấp {hijau}thành công")
                        continue
                    self.log(f"{kuning}vàng không đủ để lên cấp !")
                    break
                ...
                while True:
                    ...
                    if gold > box_price:
                        ...
                        if retcode != 200:
                            ...
                            break
                        self.log(f"{biru}mua hộp {hijau}thành công !")
                        continue
                    self.log(f"{kuning}vàng không đủ mua hộp !")
                    break
            zawarudo = True

    def main(self):
        banner = f"""
    {hijau}Auto click {biru} DJdogbot {hijau}Telegram
    
    {biru}By: {hijau}t.me/debugs0
    {biru}Github: {hijau}@bibo318
        ...
        if len(datas) == 0:
            self.log(f"{merah}không tìm thấy tài khoản nào, trước tiên hãy thêm tài khoản vào `data.txt` !")
            sys.exit()
        self.log(f"{hijau}tổng tài khoản : {putih}{len(datas)}")
        print(line)
        while True:
            for no, data in enumerate(datas):
                self.log(
                    f"{hijau}số tài khoản : {putih}{no + 1}{hijau}/{putih}{len(datas)}"
                )
